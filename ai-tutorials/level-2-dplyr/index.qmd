---
title: "Level 2 · dplyr"
description: "Twenty quests — mutate, group_by, lead/lag, joins, pivots, and tidy pipelines."
date: 2026-07-21
...
**dplyr** verbs read like sentences: select columns, filter rows, mutate new variables, summarise within groups. AI generates them fluently — and sometimes puts `filter()` after `summarise()` or forgets grouping persists. This level builds verb-by-verb mental models so you can reorder a broken pipeline in your head.

Learning progresses when you connect new syntax to problems you already solved in base R ([Bransford et al., 2000](https://nap.nationalacademies.org/catalog/9853/how-people-learn-brain-mind-experience-and-school-expanded-edition)). Each quest (~10 minutes) ends with a prediction you can check: row counts, column names, or a ggplot you recognise from the field.

Treat **dplyr** as a contract with your data: every verb documents an intent. When autocomplete proposes five chained steps, you decide whether each step is necessary — that is the scientific reasoning habit Klahr and Dunbar describe in experiment design, applied to code.

Harder verbs (**`group_by`**, **`lead`/`lag`**, **`case_when`**, **`across`**, joins, pivots) include **multiple worked examples** on reef, fishery, acoustic, and biologging tables so you recognise the verb, not just one homework dataset.
